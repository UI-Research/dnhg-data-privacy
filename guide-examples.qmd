...
temp <- parLapply(cl, 1:n, function(x) synth(nm_counts))
nm_synth <- temp[[1]]
...
temp <- lapply(1:n, function(x) synth(pa_counts))
pa_synth <- temp[[1]]
...
for(i in 2:n) {
  nm_dp5[, 2:ncol(nm_counts)] <- nm_dp5[, 2:ncol(nm_counts)] + temp[[i]][, 2:ncol(nm_counts)]
}
nm_dp5[, 2:ncol(nm_counts)] <- nm_dp5[, 2:ncol(nm_counts)] / n
...
for(i in 2:n) {
  pa_dp5[, 2:ncol(pa_counts)] <- pa_dp5[, 2:ncol(pa_counts)] + temp[[i]][, 2:ncol(pa_counts)]
}
pa_dp5[, 2:ncol(pa_counts)] <- pa_dp5[, 2:ncol(pa_counts)] / n
...
```{r}
#| label: fig-nm-box
#| fig-cap: "Boxplots for Poverty across School Districts in New Mexico"
#| warning: false
#| message: false
...
original <- data_nm %>% 
  ...
  bind_cols(Method = "Orignal")
supp <- nm_supp[[k]] %>% 
  dplyr::select(poverty) %>%
  bind_cols(Method = "Suppression")
synth <- nm_synth %>% 
  dplyr::select(poverty) %>%
  bind_cols(Method = "Synthetic")
dp <- nm_dp %>% 
  dplyr::select(poverty) %>%
  bind_cols(Method = "DP")

box_data <- bind_rows(original, supp, synth, dp) %>%
  mutate(Method = fct_relevel(Method, 
            "Original", "Suppression", "Synthetic", "DP"))

ggplot(box_data, aes(x = Method, y = poverty, fill = Method)) + geom_boxplot()
```
...
```{r}
#| label: fig-pa-box
#| fig-cap: "Boxplots for Poverty across School Districts in Pennsylvania"
#| warning: false
#| message: false
