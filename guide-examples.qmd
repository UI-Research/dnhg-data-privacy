...
# Setting the seed and number of repetitions
set.seed(20230128)
n <- 10
...
ggplot(points_data, aes(Original, Poverty)) +
  geom_abline() +
  geom_point(alpha = 0.2, aes(color = Privacy)) +
  coord_equal() +
...
bias %>%
  ...
  geom_tile() + 
  geom_text(aes(label = round(correlation, 2)), size = 4) +
  scale_fill_gradientn(colours = palette_urbn_diverging, na.value = "white", limits = c(-0.5, 0.5)) +
  theme_minimal() + 
  theme(text = element_text(size = 15), axis.text.x = element_text(angle = 40, vjust = 1, hjust = 1)) +
  labs(x = NULL, y = NULL) + 
  scale_x_discrete(labels = var) + 
...
ggplot(cor_data, aes(x = K, y = Bias, color = State)) + 
  ...
  scale_y_continuous(limits = c(0, 1.5),
                     breaks = 0:15 * 0.1) +
  labs(x = "K",
       y = "Sum of the Absolute Correlation Bias") + 
